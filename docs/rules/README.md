...
command-chain.md           ← 最上位。全 Rule の前提となるチェーン定義
  ...
  └── builder-handoff.md  ← Temporary Builder が GitHub 同期不確定の場合のハンドオフ（git-workflow の補完）
```
...
```
1. Rules      ← このディレクトリ（完成）
2. Skills     ← docs/skills/（完成）→ docs/skills/README.md
3. Workflows  ← docs/workflows/（完成）→ docs/workflows/README.md
4. RAG        ← docs/rag/（完成）→ docs/rag/README.md
5. Tools      ← docs/tools/（完成）→ docs/tools/README.md
─────────────────────────────────────
