...
- RSS fetch foundation exists but production source coverage is not verified;
- generated news JSON can be loaded by the UI;
- no delivery integration;
- OpenAI API summarization path exists, with template fallback when `OPENAI_API_KEY` is not configured.
...
3. Verify the GitHub Actions scheduled RSS update with `OPENAI_API_KEY` configured on the public repository.
