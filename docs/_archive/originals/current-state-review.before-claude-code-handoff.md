...
- RSS fetch includes timeout, retry, per-source skip, and logging; local container fetches are proxy-blocked, so GitHub Actions is the authoritative real-RSS runtime;
- generated news JSON can be loaded by the UI;
- no delivery integration;
- OpenAI API summarization path exists for summary metadata, with source-grounded template fallback when `OPENAI_API_KEY` is not configured. `importanceScore` now uses a 1-100 policy for disaster/safety, global, Japan-critical, finance, AI, and medical signals, and `confidenceReason` explains low-confidence items.
...
- live-output validation for the tuned `importanceScore` ranking;
- category-aware summarization quality tuning;
- source-grounded output is strengthened in prompts and fallback, but needs live OpenAI QA;
...
4. Review the first live generated JSON for natural ordering, confidence, and confidence reasons.
