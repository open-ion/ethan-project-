...
4. production AI ranking beyond per-article summarization;
5. Email delivery;
6. LINE delivery;
...
- `npm run summarize` generates `summary`, `whyImportant`, and `takeaway` with OpenAI when `OPENAI_API_KEY` is set, and otherwise uses a safe template fallback.
- `.github/workflows/update-news.yml` runs the same flow on a daily schedule and can commit updated generated news data.
- `.env.example` documents OpenAI and RSS settings, including `OPENAI_API_KEY`, `AI_MODEL`, and `AI_SUMMARY_LIMIT`.
