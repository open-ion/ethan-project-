...
Workflows は複数の Skills と AI 社員を組み合わせた、繰り返し実行されるプロセス定義。

| Workflow | 概要 |
| --- | --- |
| [`morning-digest`](docs/workflows/morning-digest.md) | RSS収集 → AI要約 → ランキング → Ion配信の毎朝自動フロー |
| [`pr-review`](docs/workflows/pr-review.md) | コード変更 → Guard品質チェック → Ethanレビュー → Ion報告 |
| [`new-feature`](docs/workflows/new-feature.md) | Ion要望 → 計画 → 実装 → テスト → マージ → 報告 |

Workflows 全体の索引: [`docs/workflows/README.md`](docs/workflows/README.md)
