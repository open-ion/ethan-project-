...
## AGATHON Voice Reception MVP（店舗向け電話受付Webデモ）

AGATHON LABSの最優先事業として、飲食店などの店舗向けに**電話対応・予約受付・営業電話の一次対応を受付スタッフのように行うWeb版MVP**を追加しました。電話番号連携前の営業デモとして、ブラウザ上でマイク入力またはテキスト入力を使って動作確認できます。
...
- OpenAI等の自然応答は未接続。現在は営業デモ用のルールベース応答です。
...
- 営業電話ログを予約・通常会話と分離
- 店舗用画面で予約一覧、会話ログ、営業電話ログ、店舗設定を確認・更新
- 店舗設定: 店舗名、業種、接客スタイル（restaurant/izakaya）、営業時間、定休日、住所、駐車場、FAQ、受付の口調
- 店舗設定を受付スタッフのFAQ回答・挨拶・確認文面へ反映
- Restaurant First Modeとして、飲食店受付に必要な予約・空席・駐車場・アレルギー・子ども連れ・遅刻/キャンセル連絡へ集中
- Reception Memory Engineとして、会話中に得た日時・人数・名前・電話番号・子ども連れ・アレルギー・コース・駐車場・誕生日/記念日などを保持し、不足情報だけを自然に確認
- Natural Conversation Rhythmとして、短い発話には短く返し、相づちを単調にせず、最後だけ短く確認
- Conversation Recovery Engineとして、予約途中のFAQ・言い直し・話題変更・キャンセル切替から自然に予約会話へ復帰
- Reception Personality Engineとして、初来店・常連・急ぎ・困りごと・子ども連れ・誕生日/記念日・遅刻/キャンセル・営業電話で締め言葉と温度感を変更
- Human Conversation Engineとして、相づちの連続使用を避け、情報整理・安心感・電話の締めを状況ごとに自然化

営業デモでの推奨シナリオ:

1. `/admin` で店舗名、営業時間、定休日、住所、駐車場、FAQ、口調を保存する。
2. `/voice-reception` で「明日の19時に4名で予約したいです。田中です。電話番号は090-1234-5678です。要望は窓側です。」と入力する。
3. 受付スタッフが予約内容を確認したら「はい」と入力し、仮予約完了メッセージを確認する。
...
### Restaurant Reception MVP — 今日の完成版

この版では、飲食店向け電話受付として「今日デモできる」一連の流れを実装済みです。

デモ可能な流れ:

1. `/voice-reception` で「電話着信を開始」を押す。
2. 受付スタッフとして挨拶し、用件を確認する。
