...
function renderStoreFacts(settings) {
  return `
    <div class="store-faq">
      ...
      <p><strong>通知先:</strong> ${escapeHtml(settings.notificationTo)}</p>
...
function renderVoiceAdmin() {
  ...
  const notifications = records.filter((record) => record.notification);
  const settingLabels = { name: '店舗名', businessType: '業種', hours: '営業時間', closed: '定休日', address: '住所', parking: '駐車場', seats: '席数', payment: '支払方法', privateRoom: '個室', smoking: '禁煙・喫煙', takeout: 'テイクアウト', notificationTo: '通知先メモ', faq: 'FAQ', tone: 'AI受付の口調' };
  app.innerHTML = `
    ...
    <section class="section"><div class="section-heading"><p class="eyebrow">Notifications</p><h2>スタッフ通知キュー</h2><p>外部送信前に、スタッフへ届ける内容を確認できます。</p></div><div class="news-list">${notifications.map((record) => `<article class="news-card notification-card"><div class="card-meta"><span>${escapeHtml(record.notification.title)}</span><span>${formatDate(record.createdAt)}</span></div><p><strong>通知先:</strong> ${escapeHtml(record.notification.to)}</p><p>${escapeHtml(record.notification.body)}</p><span class="status-badge status-${escapeHtml(record.notification.status || 'pending')}">送信待ち</span></article>`).join('') || '<p class="empty">通知キューはまだありません。</p>'}</div></section>
