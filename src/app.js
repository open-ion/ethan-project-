...
function renderStoreFacts(settings) {
  return `
    <div class="store-faq">
      ...
      <p><strong>お子様連れ:</strong> ${escapeHtml(settings.children || '')}</p>
      <p><strong>アレルギー:</strong> ${escapeHtml(settings.allergy || '')}</p>
      <p><strong>コース:</strong> ${escapeHtml(settings.course || '')}</p>
      <p><strong>キャンセル:</strong> ${escapeHtml(settings.cancelPolicy || '')}</p>
      <p><strong>遅刻連絡:</strong> ${escapeHtml(settings.latePolicy || '')}</p>
      <p><strong>貸切:</strong> ${escapeHtml(settings.privateBooking || '')}</p>
      <p><strong>FAQ:</strong> ${escapeHtml(settings.faq)}</p>
      <p><strong>接客スタイル:</strong> ${escapeHtml(receptionStyleProfiles[settings.receptionStyle]?.label || settings.receptionStyle || '居酒屋')}</p>
...
function renderVoiceReception() {
  ...
  app.innerHTML = `
    <section class="hero voice-hero">
      ...
      <h1>${escapeHtml(settings.name)}の電話受付デモ</h1>
      <p class="hero-lead">電話着信 → 受付挨拶 → 用件確認 → 予約/FAQ/営業電話/人への引き継ぎ → 終了まで、今日デモできる店舗向け商品MVPです。</p>
    ...
    <section class="section voice-console">
      <div class="section-heading"><p class="eyebrow">Reception Console</p><h2>受付会話</h2><p>予約例:「明日の19時に4名で予約したいです。田中です。電話番号は090-1234-5678です。席は窓側希望です。」→ 受付スタッフが復唱 →「はい」で予約受付完了。</p></div>
      <div class="voice-grid">
        <div class="voice-panel">
          <label for="voice-input"><strong>お客様の発話</strong></label>
          <textarea id="voice-input" class="idea-input" rows="5" placeholder="予約、FAQ、営業電話、スタッフへの取次ぎなどをお話しください"></textarea>
          <div class="idea-actions"><button type="button" class="primary-link" data-start-voice>🎙️ マイク入力</button><button type="button" class="secondary-link" data-send-voice>受付スタッフへ渡す</button></div>
        ...
        <div class="voice-panel"><strong>受付スタッフ応答</strong><p class="voice-reply" data-voice-reply>電話着信を開始すると受付スタッフが挨拶します。</p>${renderStoreFacts(settings)}</div>
      </div>
    </section>`;
}

function getStatusLabel(status) {
  return ({ unconfirmed: '未確認', confirmed: '確認済み', needs_confirmation: '確認待ち', draft: '受付中', ai_confirmed: '受付済み', retrying: '聞き返し中' }[status] || status || '未確認');
}
...
function renderVoiceAdmin() {
  ...
  const settingLabels = { name: '店舗名', businessType: '業種', receptionStyle: '接客スタイル', hours: '営業時間', closed: '定休日', address: '住所', parking: '駐車場', seats: '席数', payment: '支払方法', privateRoom: '個室', smoking: '禁煙・喫煙', takeout: 'テイクアウト', children: 'お子様連れ', allergy: 'アレルギー', course: 'コース', cancelPolicy: 'キャンセル確認', latePolicy: '遅刻連絡', privateBooking: '貸切相談', faq: 'FAQ', tone: '受付の口調' };
  app.innerHTML = `
    <section class="hero"><p class="eyebrow">Store Admin</p><h1>電話受付 管理画面</h1><p class="hero-lead">予約、会話ログ、営業電話ログ、人への転送、店舗設定を1画面で確認できます。現在はlocalStorage保存のプロトタイプです。</p><div class="hero-actions"><a class="primary-link" href="./voice-reception">受付デモへ戻る</a></div></section>
    ...
    <section class="section"><div class="section-heading"><p class="eyebrow">Reservations</p><h2>予約一覧</h2><p>受付済み予約はスタッフ確認後に「確認済み」へ変更できます。</p></div><div class="admin-table">${reservations.map((record) => `<article class="admin-row reservation-row"><strong>${escapeHtml(record.reservation?.name || '名前未取得')}</strong><span>${escapeHtml(record.reservation?.dateTime || '日時未取得')}</span><span>${escapeHtml(record.reservation?.people || '人数未取得')}</span><span>${escapeHtml(record.reservation?.phone || '電話未取得')}</span><span>${escapeHtml(record.reservation?.seatType || '席指定なし')}</span><span>${escapeHtml(record.reservation?.request || '要望なし')}</span><span class="status-badge status-${escapeHtml(record.status || 'unconfirmed')}">${getStatusLabel(record.status)}</span><button type="button" class="secondary-link compact-action" data-confirm-record="${record.id}">確認済み</button><small>${formatDate(record.createdAt)}</small></article>`).join('') || '<p class="empty">予約ログはまだありません。</p>'}</div></section>
    ...
    <section class="section"><div class="section-heading"><p class="eyebrow">Conversation Logs</p><h2>会話ログ</h2><p>${conversations.length}件</p></div><div class="news-list">${conversations.map((record) => `<article class="news-card"><div class="card-meta"><span>${escapeHtml(record.intent)}</span><span>${formatDate(record.createdAt)}</span></div><h3>お客様</h3><p>${escapeHtml(record.transcript)}</p><h3>受付スタッフ</h3><p>${escapeHtml(record.reply)}</p></article>`).join('') || '<p class="empty">会話ログはまだありません。</p>'}</div></section>
    <section class="section"><div class="section-heading"><p class="eyebrow">Store Settings</p><h2>店舗設定</h2><p>受付スタッフの話し方に即時反映されます。</p></div><form class="settings-form" data-store-settings>${Object.keys(settingLabels).map((key) => key === 'receptionStyle'
      ? `<label><span>${escapeHtml(settingLabels[key])}</span><select name="${key}">${Object.entries(receptionStyleProfiles).map(([value, profile]) => `<option value="${escapeHtml(value)}" ${settings.receptionStyle === value ? 'selected' : ''}>${escapeHtml(profile.label)}</option>`).join('')}</select></label>`
      : `<label><span>${escapeHtml(settingLabels[key])}</span><textarea name="${key}" rows="${key === 'faq' || key === 'tone' ? 3 : 2}">${escapeHtml(settings[key] ?? defaultStoreSettings[key] ?? '')}</textarea></label>`).join('')}<button type="submit" class="primary-link">店舗設定を保存</button><small class="idea-note" data-settings-note>保存すると受付画面の応答に反映されます。</small></form></section>`;
}
...
function bindVoiceEvents() {
  ...
  const send = () => {
    ...
    if (!transcript) { if (status) status.textContent = 'お客様の発話が空です。少しお話しいただいてからお試しください。'; return; }
    ...
    if (status) status.textContent = `記録しました（${intent}） / 現在の状態: ${session.stage} / ${formatReservationSummary(session.reservation)}`;
  ...
  app.querySelector('[data-start-voice]')?.addEventListener('click', () => {
    ...
    if (!SpeechRecognition) { if (status) status.textContent = 'このブラウザは音声入力に未対応です。テキスト欄でお試しください。'; return; }
    const recognition = new SpeechRecognition();
    recognition.lang = 'ja-JP';
    recognition.interimResults = false;
    recognition.onstart = () => { if (status) status.textContent = '聞き取り中です...'; };
    recognition.onerror = () => { if (status) status.textContent = '音声入力に失敗しました。テキスト欄でお試しください。'; };
    recognition.onresult = (event) => { input.value = event.results[0][0].transcript; if (status) status.textContent = '聞き取り完了。受付スタッフへ渡せます。'; };
    recognition.start();
  });
}
